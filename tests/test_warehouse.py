...
class AWarehouseTestCase( TestCase ):
    ...
    def test_take_put_items( self ):
        ...
        self.assertEqual( w_house.TakeItems( items_level=1, number_of_items=15 ), 15 )

        w_house.PutItems( items_level=2, number_of_items=200 )
        self.assertEqual( w_house.amount, 100 )
        self.assertEqual( w_house.TakeItems( items_level=1, number_of_items=10 ), 5 )
        self.assertEqual( w_house.amount, 95 )
        self.assertEqual( w_house.TakeItems( items_level=1, number_of_items=10 ), 0 )
        self.assertEqual( w_house.amount, 95 )
        self.assertEqual( w_house.TakeItems( items_level=-1, number_of_items=10 ), 0 )
        self.assertEqual( w_house.amount, 95 )
        self.assertEqual( w_house.TakeItems( items_level=10, number_of_items=10 ), 0 )
        self.assertEqual( w_house.amount, 95 )
        self.assertEqual( w_house.TakeItems( items_level=2, all_items=True ), 95 )
        self.assertEqual( w_house.amount, 0 )
